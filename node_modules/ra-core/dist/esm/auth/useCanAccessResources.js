...
export var useCanAccessResources = function (params) {
    ...
    var action = params.action, resources = params.resources;
    var queryResult = useQueries({
        queries: resources.map(function (resource) {
            return {
        ...
        combine: (combineSourceAccessResults),
    });
...
var combineSourceAccessResults = function (results) {
    var _a;
    return {
        data: results
            ? results.reduce(function (acc, _a) {
                var _b;
                var data = _a.data;
                if (!data) {
                    return acc;
                }
                var resource = data.resource, canAccess = data.canAccess;
                return __assign(__assign({}, acc), (_b = {}, _b[resource] = canAccess, _b));
            }, {})
            : undefined,
        isPending: results.some(function (result) { return result.isPending; }),
        isError: results.some(function (result) { return result.isError; }),
        error: ((_a = results.find(function (result) { return result.error; })) === null || _a === void 0 ? void 0 : _a.error) || undefined,
    };
};
